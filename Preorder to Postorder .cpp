...
Node* constructtreeutill(int pre[],int size,int min,int max,int *index,int val)
{
    if(*index>=size)
    {
        return NULL;
    ...
    if((val>min)&&(val<max))
    {
        ...
        *index=*index+1;
        if(*index<size)
        {
            root->left=constructtreeutill(pre,size,min,val,index,pre[*index]);
        }
        if(*index<size)
        {
            root->right=constructtreeutill(pre,size,val,max,index,pre[*index]);
        }
    }
    return root;
}
Node* constructTree(int pre[], int size) {
    int index=0;
    return constructtreeutill(pre,size,INT_MIN,INT_MAX,&index,pre[0]);
}
